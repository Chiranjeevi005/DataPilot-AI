...
export default function DonutChartCard({ data, title = "Distribution", nameKey = "name", dataKey = "value" }: DonutChartCardProps) {
        ...
        <div className="bg-datapilot-card rounded-[12px] shadow-soft p-6 border border-datapilot-border flex flex-col h-[320px] transition-all hover:shadow-card-hover group relative">
            ...
            <div className="flex-1 w-full min-h-0 relative">
                <div className="absolute inset-0 flex items-center justify-center flex-col pointer-events-none">
                    <span className="text-datapilot-muted text-xs font-medium">Total</span>
                    <span className="text-datapilot-text text-xl font-bold">{total.toLocaleString()}</span>
                </div>
